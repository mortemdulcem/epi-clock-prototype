...
def read_table(
    ...
    **kwargs,
    ...
    """
    Genel amaçlı tablo okuma yardımcı fonksiyonu.

    Parametreler
    - path: Dosya yolu.
    - fmt: Zorla format belirtimi. None ise uzantıdan çıkarılır.
    - kwargs: pandas/pyarrow reader argümanları.
...
def write_table(
    ...
    **kwargs,
    ...
    """
    DataFrame'i farklı formatlara yazma yardımcı fonksiyonu.
    """
    p = Path(path)
    if fmt is None:
        ext = p.suffix.lower().lstrip(".")
