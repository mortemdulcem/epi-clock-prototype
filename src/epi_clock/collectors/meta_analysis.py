...
class MetaAnalyzer:
    ...
    def _extract_key_terms(self, df: pd.DataFrame) -> Dict:
        ...
        addiction_terms = [
            'cocaine', 'alcohol', 'nicotine', 'opioid', 'cannabis', 'methamphetamine',
            'substance abuse', 'drug addiction', 'alcoholism', 'smoking'
        ]
        
        # Key methylation terms  
        methyl_terms = [
            'DNA methylation', 'CpG sites', 'hypermethylation', 'hypomethylation',
            'methylation patterns', 'epigenetic modifications', 'methylome'
        ]
    ...
    def _identify_high_impact(self, df: pd.DataFrame, min_citations: int = 50) -> List[Dict]:
    ...
    def synthesize_findings(self, literature_analysis: Dict, geo_analysis: Dict) -> Dict:
        ...
        synthesis = {
            ...
            'methodological_insights': self._extract_methodological_insights(literature_analysis),
            'data_availability': self._assess_data_availability(geo_analysis),
            'research_gaps': self._identify_research_gaps(literature_analysis, geo_analysis),
            'recommendations': self._generate_recommendations(literature_analysis, geo_analysis)
        }
    ...
    def _extract_methodological_insights(self, lit_analysis: Dict) -> List[str]:
        ...
        if 'study_types' in lit_analysis:
            ...
            insights.append(f"Most common study design: {most_common[0]} ({most_common[1]} studies)")
    ...
    def _assess_data_availability(self, geo_analysis: Dict) -> List[str]:
        ...
        if 'cpg_overlap' in geo_analysis and geo_analysis['cpg_overlap']:
            ...
            availability.append(f"Core CpG sites across datasets: {core_cpgs}/{total_cpgs}")
        
        quality_issues = []
        if 'data_quality' in geo_analysis:
            for acc, quality in geo_analysis['data_quality'].items():
                if quality['missing_rate'] > 0.1:
                    quality_issues.append(f"{acc}: {quality['missing_rate']:.1%} missing")
        
        if quality_issues:
            availability.append(f"Data quality concerns: {len(quality_issues)} datasets with >10% missing")
        
        return availability
    
    def _identify_research_gaps(self, lit_analysis: Dict, geo_analysis: Dict) -> List[str]:
        ...
        if 'study_types' in lit_analysis:
            ...
            longitudinal_pct = study_types.get('longitudinal', 0) / sum(study_types.values())
        ...
        # CpG overlap gap
        if 'cpg_overlap' in geo_analysis and geo_analysis['cpg_overlap']:
            core_ratio = (geo_analysis['cpg_overlap'].get('core_cpgs', 0) / 
                         geo_analysis['cpg_overlap'].get('total_unique_cpgs', 1))
            if core_ratio < 0.3:
                gaps.append("Low CpG site overlap between datasets limits meta-analysis")
        
        return gaps
    
    def _generate_recommendations(self, lit_analysis: Dict, geo_analysis: Dict) -> List[str]:
        """Generate recommendations for future research"""
        recommendations = []
        
        recommendations.append("Standardize methylation platforms for better data integration")
        recommendations.append("Increase sample sizes through multi-site collaborations")
        ...
        if 'key_terms' in lit_analysis:
            ...
            if missing:
                recommendations.append(f"Address understudied substances: {', '.join(missing)}")
