...
class GEOCollector:
        ...
        try:
            ...
            for gsm_name, gsm in gse.gsms.items():
                ...
                if hasattr(gsm, "table") and gsm.table is not None:
                    ...
                    if (
                        "VALUE" in gsm.table.columns
                        or "Beta_value" in gsm.table.columns
                    ):
            ...
            if methylation_matrices:
                print(
                    f"[GEO] Processing methylation data for {len(methylation_matrices)} samples"
                )
                dataset_info["methylation_data"] = self._combine_methylation_data(
                    methylation_matrices
                )
    ...
    def _parse_characteristics(self, metadata: Dict) -> Dict:
        ...
        char_fields = [
            key for key in metadata.keys() if "characteristics" in key.lower()
        ]
    ...
    def extract_cpg_sites(self, datasets: Dict[str, Dict]) -> pd.DataFrame:
        ...
        return pd.DataFrame(cpg_summary).sort_values(
            "present_in_datasets", ascending=False
        )
