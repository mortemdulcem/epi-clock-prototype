import argparse
from . import ingest, features, train, evaluate, make_figures
from .collectors import pubmed_collector, geo_collector, meta_analysis
...
def collect_pubmed_data(config_path: str):
    ...
    print(f"[collect_pubmed] Saved {len(df)} studies to {output_dir}/pubmed_studies.csv")
...
def main():
    p = argparse.ArgumentParser(description="Epigenetic Clock Pipeline with Real Data Collection")
    sub = p.add_subparsers(dest="cmd", required=True)
    
    # Data collection commands
    collect_pubmed_cmd = sub.add_parser("collect_pubmed", help="Collect PubMed literature data")
    collect_pubmed_cmd.add_argument("--config", required=True)
    
    collect_geo_cmd = sub.add_parser("collect_geo", help="Collect GEO methylation datasets")
    collect_geo_cmd.add_argument("--config", required=True)
    
    meta_cmd = sub.add_parser("meta_analyze", help="Run meta-analysis on collected data")
